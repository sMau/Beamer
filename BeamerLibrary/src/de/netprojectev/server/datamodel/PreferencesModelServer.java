...
public class PreferencesModelServer {
	...
	public void deserializeAll() {
		...
		HashMap<UUID, MediaFileServer> allMedia = null;
		try {
			allMedia = (HashMap<UUID, MediaFileServer>) HelperMethods.loadFromFile(ConstantsServer.FILENAME_MEDIAFILES, ConstantsServer.SAVE_PATH);
	...
	public void removePriority(UUID id) throws IOException {
		...
		for (UUID key : this.proxy.getMediaModel().getAllMediaFiles().keySet()) {
			MediaFileServer curFile = this.proxy.getMediaModel().getAllMediaFiles().get(key);
			if (curFile.getPriorityID().equals(id)) {
				curFile.setPriority(defaultPriority);
				this.proxy.broadcastMessage(new Message(OpCode.STC_EDIT_MEDIA_FILE_ACK, new MediaFileClient(curFile)));
			}
		}
